exports.encryptRailFence = (text, key) => {
  ...
  if(key > Math.floor(2*(plaintext.length-1))){ return "key is too large" }  
  if(key==1) railFenceText = plaintext;
    ...
    for(i=line; i<plaintext.length; i+=2*(key-1)) railFenceText += plaintext.charAt(i);
  }
...
exports.decryptRailFence = (railFenceText, key) => {
  ...
  var key = parseInt(key)
  if(key > Math.floor(2*(ciphertext.length-1))){ return "key is too large for the ciphertext length." }      
  if(key==1) return ciphertext;
  else{
    text = new Array(ciphertext.length);   k=0;
    for(line=0; line<key-1; line++){
      skip=2*(key-line-1);	 j=0;
    ...
    for(i=line; i<ciphertext.length; i+=2*(key-1)) text[i] = ciphertext.charAt(k++);
