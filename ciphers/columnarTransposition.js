exports.encryptColumnarTransposition = (text, key, padChar = 'x') => {
    ...
    for(i=0; i < key.length; i++){
        ...
        for(j=0; j < colLength; j++) columnarTranspositionText += plaintext.charAt(j*key.length + t);
    }
...
exports.decryptColumnarTransposition = (columnarTranspositionText, key) => {
    ...
    klen = keyword.length;
    if(klen <= 1){ return "keyword should be at least 2 characters long" }
    if(ciphertext.length % klen != 0){ return "ciphertext has not been padded, the result may be incorrect (incorrect keyword?)." }
